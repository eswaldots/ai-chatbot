...
export const getWeather = tool({
  ...
  execute: async (input) => {
    ...
    if ("city" in input.query) {
      const coords = await geocodeCity(input?.query?.city);
      if (!coords) {
        return {
          error: `Could not find coordinates for "${input.query.city}". Please check the city name.`,
        };
      }
      latitude = coords.latitude;
      longitude = coords.longitude;
    } else {
      latitude = input.query.latitude;
      longitude = input.query.longitude;
    }
